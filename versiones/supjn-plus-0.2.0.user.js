 ...
 *
 * QUÉ ES
 *   Una sola ventana, movible, que se abre sola en la Consulta Web del PJN y
 *   junta todo: Mis causas (Relacionados) y Favoritos con búsqueda, filtros,
 *   orden y columnas movibles; etiquetas y anotaciones propias; dejar nota en
 *   todas o en las seleccionadas; bajar expedientes en un PDF, eligiendo
 *   causas desde la lista o actuaciones desde el expediente; y un menú con las
 *   demás funciones del PJN (escritos, DEOX, notificaciones y el resto).
 *
 ...
 *       lleva al Sistema de Escritos: se abre en una pestaña nueva con el
 *       propio enlace del PJN.
 *
...
(function () {
  ...
  const S_CORRIDA = 'notatomic_corrida';  // sessionStorage: tanda de notas en curso
  ...
  const COLS_DEF = [
    { k: 'exp', t: 'Expediente', w: 158 },
    { k: 'dep', t: 'Dependencia', w: 230 },
    { k: 'car', t: 'Carátula', w: 330 },
    { k: 'sit', t: 'Situación', w: 118 },
    { k: 'ult', t: 'Últ. act.', w: 96 },
    { k: 'nota', t: 'Nota', w: 108 },
    { k: 'et', t: 'Etiquetas', w: 160 },
    { k: 'anot', t: 'Anotación', w: 190 }
  ];
  const COL = {};
  ...
  function ordenColumnas() {
    const vistas = {};
    const out = [];
    (Array.isArray(CFG.cols) ? CFG.cols : []).forEach((k) => { if (COL[k] && !vistas[k]) { vistas[k] = true; out.push(k); } });
    COLS_DEF.forEach((c) => { if (!vistas[c.k]) out.push(c.k); });
    return out;
  }
  const columnasVisibles = () => ordenColumnas().filter((k) => CFG.ocultas.indexOf(k) < 0);
  const anchoDe = (k) => Math.max(60, parseInt(CFG.anchos[k], 10) || COL[k].w);

  function moverColumna(k, destino, despues) {
    const orden = ordenColumnas().filter((x) => x !== k);
  ...
  const datosVista = () => DATOS[VISTA === 'fav' ? 'fav' : 'rel'];
  const selVista = () => SEL[VISTA === 'fav' ? 'fav' : 'rel'];
  ...
  const CSS = [
    ...
    '#supjn.maxi{left:0!important;top:0!important;width:100vw!important;height:100vh!important;border-radius:0}',
  ...
  function pintarSolapas() {
    ...
    s.push(['marcas', 'Etiquetas y respaldo', null]);
    s.push(['acerca', 'Acerca de', null]);
    e.innerHTML = s.map(([k, t, n]) => '<button data-vista="' + k + '" class="' + (VISTA === k ? 'act' : '') + '">' + esc(t) +
  ...
  function pintarAccion() {
    ...
    e.innerHTML = '<span class="cuenta">' + (n ? plural(n, 'seleccionada', 'seleccionadas') : 'Ninguna seleccionada') + '</span>' +
      '<button class="sj-b" data-a="notaSel"' + dis(!n || b) + '>Dejar nota en las seleccionadas</button>' +
      ...
      '<span class="der"><button class="sj-b" data-a="notaTodas"' + dis(b) + '>Dejar nota en todas</button></span>';
  }

  function pintarConfirma() {
    const e = q('[data-e="confirma"]');
    if (!e) return;
    if (!confirmaNota || !esVistaLista()) { e.style.display = 'none'; e.innerHTML = ''; return; }
    const solo = confirmaNota.solo;
    const fueraRel = solo ? solo.filter((k) => !causaEn('rel', k)).length : 0;
    e.style.display = '';
    e.innerHTML = '<span>' + (solo
      ? 'Vas a dejar nota en <b>' + plural(solo.length, 'causa elegida', 'causas elegidas') + '</b>' + (solo.length <= 3 ? ' (' + solo.map(esc).join(', ') + ')' : '')
      : 'Vas a dejar nota en <b>todas las causas que el PJN habilite hoy</b>') +
      '. Se hace en la lista de Relacionados del PJN y la página se recarga una vez por nota.' +
      ...
      '<button class="sj-b" data-a="cancelarNota">Cancelar</button>';
  }
  ...
  function pintarTabla() {
    ...
    const ANCHO_SEL = 34, ANCHO_ACC = 112;
    const total = ANCHO_SEL + ANCHO_ACC + cols.reduce((s, k) => s + anchoDe(k), 0);
    ...
    const cab = '<th class="cs fija"><input type="checkbox" data-a="selTodas" title="Seleccionar o quitar las ' + L.length + ' causas filtradas"' + (todasSel ? ' checked' : '') + '></th>' +
      cols.map((k) => '<th class="mov' + (col === k ? ' act' : '') + '" draggable="true" data-k="' + k + '" title="Clic para ordenar. Arrastrá el título para mover la columna y su borde derecho para cambiar el ancho.">' +
        esc(COL[k].t) + '<span class="fl">' + (col === k ? (desc ? '▼' : '▲') : '▽') + '</span><span class="rs" data-rs="' + k + '"></span></th>').join('') +
      '<th class="fija"></th>';
    ...
    const filas = trozo.map((c) => {
      ...
      return '<tr class="' + (c.tramite ? '' : 'fuera') + (s ? ' sel' : '') + '">' +
        ...
        '<td><div class="sj-acc"><button class="sj-abrir" data-abrir="' + esc(c.exp) + '" title="Abre el expediente en esta pestaña">Abrir</button>' +
        '<button class="sj-mas" data-mas="' + esc(c.exp) + '" title="Más acciones: pestaña nueva, libro digital, presentar escrito, bajar, dejar nota">⋯</button></div></td>' +
        '</tr>' + (abierta === c.exp ? detalleHTML(c, cols.length + 2) : '');
    }).join('');

    cuerpo.innerHTML = '<table class="sj-t" style="width:' + total + 'px;min-width:100%"><colgroup><col style="width:' + ANCHO_SEL + 'px">' +
      cols.map((k) => '<col data-col="' + k + '" style="width:' + anchoDe(k) + 'px">').join('') + '<col style="width:' + ANCHO_ACC + 'px"></colgroup>' +
  ...
  function abrirMenu(ancla, html) {
    ...
    let left = ra.left - rw.left;
    if (left + ancho > rw.width - 8) left = Math.max(8, ra.right - rw.left - ancho);
    let top = ra.bottom - rw.top + 4;
    if (top + alto > rw.height - 8) top = Math.max(8, ra.top - rw.top - alto - 4);
  ...
  function menuColsHTML() {
    return '<div class="tit">Columnas visibles</div>' +
      ordenColumnas().map((k) => '<label class="it"><input type="checkbox" data-colvis="' + k + '"' + (CFG.ocultas.indexOf(k) < 0 ? ' checked' : '') + '> ' + esc(COL[k].t) + '</label>').join('') +
      '<div class="sep"></div><button class="it" data-a="colsReset">Restablecer orden, anchos y columnas</button>' +
      '<div class="tit" style="text-transform:none;font-weight:400;white-space:normal;max-width:260px">Para mover una columna arrastrá su título; para cambiar el ancho, su borde derecho.</div>';
  }
  ...
  function elegirHTML(id) {
    ...
    const filas = V.map((i) => {
      const a = ctx.acts[i];
      return '<tr><td style="width:28px"><input type="checkbox" data-ei="' + i + '"' + (ctx.elegidas.has(i) ? ' checked' : '') + '></td>' +
        '<td class="f">' + esc(a.fecha) + '</td><td class="t">' + esc(a.tipo) + (a.hist ? ' <span class="sj-badge">histórica</span>' : '') + '</td>' +
        '<td>' + esc(a.detalle) + '</td><td class="f">' + esc(a.fojas) + '</td>' +
        '<td style="width:40px">' + (a.ver ? '<a href="' + esc(a.ver) + '" target="_blank" rel="noopener noreferrer">Ver</a>' : '') + '</td></tr>';
    }).join('');
    return '<div class="sj-elegir" data-elegir="' + esc(id) + '">' +
      ...
      '<div class="lst"><table><tbody>' + (filas || '<tr><td class="sj-vacio">' + (ctx.acts.length ? 'Ninguna actuación coincide con el filtro.' : 'No hay actuaciones con PDF.') + '</td></tr>') + '</tbody></table></div>' +
  ...
  function panelAcercaHTML() {
    return '<div class="sj-panel-in">' +
      ...
      '<p>Una sola ventana sobre la Consulta Web del PJN. Se abre sola, se mueve arrastrando la barra azul, se agranda desde la esquina de abajo a la derecha y tiene minimizar, maximizar y cerrar.</p>' +
      '<h3>Qué hace</h3>' +
      '<p><b>Mis causas y Favoritos:</b> lee las dos listas del PJN, en trámite y fuera de trámite, en segundo plano y sin mover la página que estás viendo. Se busca, se filtra, se ordena por cualquier columna y las columnas se mueven, se ensanchan y se ocultan.</p>' +
      '<p><b>Dejar nota:</b> en todas las causas que el PJN habilite o solo en las seleccionadas. Pide confirmar antes de empezar y guarda el resultado de cada causa en la columna Nota.</p>' +
  ...
  // "Presentar escrito" lleva a otra aplicación del PJN: se usa el propio
  // enlace de la fila, dirigido a una pestaña nueva.
  async function presentarEscrito(k) {
    if (ocupado()) return;
    const nombre = 'supjn_escrito_' + Date.now();
    const w = pestanaNueva(nombre, k);
    if (!w) return;
    accionEnCurso = true;
    try {
      ...
      if (!form) throw new Error('no encuentro el formulario del PJN');
      const antes = form.getAttribute('target');
      form.setAttribute('target', nombre);
      try {
        a.click();
      } finally {
        if (antes == null) form.removeAttribute('target'); else form.setAttribute('target', antes);
        try { const P = u.fr.contentWindow.PF; if (P && P('dialog')) P('dialog').hide(); } catch (e) { /* sin cartel */ }
      }
      soltarMarcoTrabajo();
  ...
  function acomodarVentana() {
    if (!win || CFG.maxi || win.style.display === 'none') return;
    const r = win.getBoundingClientRect();
    if (r.width > innerWidth - 8) win.style.width = Math.max(520, innerWidth - 8) + 'px';
    if (r.height > innerHeight - 8) win.style.height = Math.max(320, innerHeight - 8) + 'px';
    const r2 = win.getBoundingClientRect();
    if (r2.left > innerWidth - 120) win.style.left = Math.max(4, innerWidth - r2.width - 4) + 'px';
    if (r2.top > innerHeight - 40) win.style.top = Math.max(4, innerHeight - r2.height - 4) + 'px';
  }
  ...
  function ubicarVentana() {
    ...
    win.style.top = Math.max(4, Math.round((H - h) / 2)) + 'px';
  ...
  function construir() {
    ...
    win.innerHTML =
      ...
      '<div class="sj-confirma" data-e="confirma" style="display:none"></div>' +
    ...
    q('[data-e="tit"]').addEventListener('mousedown', (e) => {
      ...
      arrastrar(e, (ev) => {
        const l = Math.max(120 - r.width, Math.min(innerWidth - 120, r.left + ev.clientX - sx));
        const t = Math.max(0, Math.min(innerHeight - 38, r.top + ev.clientY - sy));
    ...
    q('[data-e="redim"]').addEventListener('mousedown', (e) => {
      ...
      const sx = e.clientX, sy = e.clientY;
      arrastrar(e, (ev) => {
        win.style.width = Math.round(Math.max(520, Math.min(innerWidth - r.left - 2, r.width + ev.clientX - sx))) + 'px';
        win.style.height = Math.round(Math.max(320, Math.min(innerHeight - r.top - 2, r.height + ev.clientY - sy))) + 'px';
      }, () => {
        CFG.tam[TIPO_PAG] = { w: Math.round(win.offsetWidth), h: Math.round(win.offsetHeight) };
        guardarCfg();
      });
    });
    ...
    win.addEventListener('mousedown', (e) => {
      ...
      const k = rs.dataset.rs;
      const th = rs.closest('th');
      const colEl = q('col[data-col="' + k + '"]');
      const tabla = th.closest('table');
      const ancho0 = anchoDe(k), total0 = parseInt(tabla.style.width, 10) || tabla.offsetWidth;
      const sx = e.clientX;
      th.draggable = false;
      recienRedim = true;
      arrastrar(e, (ev) => {
        const w = Math.max(60, Math.round(ancho0 + ev.clientX - sx));
        if (colEl) colEl.style.width = w + 'px';
        tabla.style.width = (total0 + w - ancho0) + 'px';
        CFG.anchos[k] = w;
      }, () => {
        th.draggable = true;
        guardarCfg();
    ...
    win.addEventListener('dragstart', (e) => {
      ...
      if (!th) return;
      colArrastrada = th.dataset.k;
      th.classList.add('arrastrando');
      try { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', colArrastrada); } catch (x) { /* sin datos */ }
    });
    ...
    win.addEventListener('dragover', (e) => {
      ...
      limpiarDrop();
      if (th.dataset.k !== colArrastrada) th.classList.add(der ? 'drop-der' : 'drop-izq');
    });
    win.addEventListener('drop', (e) => {
      if (!colArrastrada) return;
      const th = e.target.closest && e.target.closest('th[data-k]');
      e.preventDefault();
      const k = colArrastrada;
      colArrastrada = null;
      limpiarDrop();
      if (!th || th.dataset.k === k) { pintarTabla(); return; }
      const r = th.getBoundingClientRect();
      moverColumna(k, th.dataset.k, e.clientX > r.left + r.width / 2);
      pintarTabla();
    });
    ...
    win.addEventListener('change', (e) => {
      ...
      if (t.matches('[data-colvis]')) {
        const k = t.dataset.colvis;
        CFG.ocultas = CFG.ocultas.filter((x) => x !== k);
        if (!t.checked) {
          if (columnasVisibles().length <= 1) { t.checked = true; return; }
          CFG.ocultas.push(k);
        }
        guardarCfg();
        pintarTabla();
    ...
    document.addEventListener('mousedown', (e) => {
      ...
      if (e.target.closest && e.target.closest('[data-a="menuPJN"], [data-a="menuCols"], [data-mas]')) return;
      cerrarMenu();
    }, true);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (confirmaNota) { confirmaNota = null; pintarConfirma(); return; }
      if (abierta) { guardarNotaAbierta(); abierta = null; pintarTabla(); return; }
      minimizar();
    });

    window.addEventListener('resize', acomodarVentana);
  ...
  function alClic(e) {
    ...
    if (th && !t.closest('[data-rs]')) {
      ...
      else CFG.orden = { col: k, desc: k === 'ult' || k === 'nota' };
      guardarCfg();
      pintarTabla();
      return;
    }
    ...
    if (ab) { abrirCausa(ab.dataset.abrir, false); return; }
    ...
    switch (a) {
      ...
      case 'menuPJN':
        ...
        return;
      case 'menuCols':
        if (menuAbierto && menuAbierto.dataset.ancla === 'menuCols') { cerrarMenu(); return; }
        abrirMenu(b, menuColsHTML());
        return;
      case 'colsReset':
        CFG.cols = COLS_DEF.map((c) => c.k);
        CFG.ocultas = [];
        CFG.anchos = {};
        guardarCfg();
        pintarTabla();
      ...
      case 'quitarSel': selVista().clear(); pintarTabla(); pintarAccion(); return;
      ...
      case 'cancelarNota': confirmaNota = null; pintarConfirma(); return;
  ...
  function arrancar() {
    ...
    // Pedido del autor para SuPJN+: que cargue de una, con la ventana abierta.
    abrirVentana();
