...
(function () {
  ...
  function cedulaEnNotif() {
    ...
    // El campo de jurisdicción es un desplegable de Material UI. Comprobado
    // sobre el formulario real del PJN: al cargar la página el campo toma el
    // foco y la lista se abre sola, y cada clic en el campo la abre o la
    // cierra según como esté. Por eso lo primero, siempre, es mirar si ya está
    // abierta: insistir a ciegas la cerraba justo antes de leerla, y ese era
    // el motivo de "no dejó abrir la lista de jurisdicción".
    //
    // Las opciones tardan unas décimas en dibujarse, así que después de cada
    // intento se espera. Y como el PJN cambia el formulario sin avisar, se
    // guardan las cuatro formas conocidas de abrirlo, pero se usan de a una y
    // solo cuando la lista está cerrada: el botón de la flecha, el clic en el
    // campo, la tecla de abajo y escribir la sigla.
    async function elegirCamara(cam) {
      ...
      let vioLista = false, vioVacia = false;
      ...
      for (let vuelta = 0, f = 0; vuelta < formas.length + 2 && Date.now() < tope; vuelta++) {
        ...
        cam.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
        cam.blur();
        if (sigla(cam.value) === p.sigla) return 'elegida';
        await cerrar();
      }
      return vioLista ? 'sin opción' : vioVacia ? 'lista vacía' : 'sin lista';
    }
    ...
    (async () => {
      ...
      if (r0.camara !== 'elegida') {
        // Se dice cuál de las cuatro cosas pasó: el formulario no mostró la
        // lista, la mostró vacía, la mostró y ninguna opción trae la sigla, o
        // trae más de una. Eso es lo que hace falta para arreglarlo sin
        // adivinar, porque cada una se arregla en un lugar distinto.
        ...
        cartel('se cargaron el número y el año de ' + buscado + ', pero ' + porQue + '. ' +
          (r0.camara === 'lista vacía'
            ? 'Recargá la página: si vuelve vacía, es del PJN y no de SuPJN+.'
            : 'Elegí ' + p.sigla + ' a mano y pulsá Siguiente.'), true);
        return;
      }
      let r = '';
