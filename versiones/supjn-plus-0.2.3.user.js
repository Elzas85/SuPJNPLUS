 ...
 *   apuntes propios; dejar nota en todas o en las seleccionadas; bajar
...
(function () {
  ...
  const FUEROS_PENALES = ['CCC', 'CFP', 'CPE', 'CCF', 'FLP', 'FMP', 'FCB', 'FRO', 'FSM'];
  ...
  function partesDeCaratula(exp, car) {
    ...
    const rol = /sucesi/i.test(objeto) ? 'Causante'
      : (FUEROS_PENALES.indexOf(fueroDe(exp)) >= 0 || OBJETO_PENAL.test(objeto)) ? 'Imputado'
        : objeto ? 'Parte' : '';
  ...
  function pintarExpediente() {
    ...
    p.innerHTML =
      ...
      (partesDe({ exp: k, car: d.car }).length ? '<div class="p">' + partesHTML(partesDe({ exp: k, car: d.car })) + '</div>' : '') +
  ...
  if (location.hostname === 'pruebas.supjn.invalid' && typeof window.__SUPJN_PRUEBAS__ === 'function') {
    window.__SUPJN_PRUEBAS__({
      ...
      enterosExactos, anchosEncuadrados, repartirEncuadre, minCol, anchoDe, columnasVisibles, CFG, COLS_DEF, COLS_ACT_DEF,
