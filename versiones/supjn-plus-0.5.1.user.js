...
(function () {
  ...
  const ARCHIVO_RESPALDO = 'SuPJN+-datos.json';
  let CARPETA = null;            // la carpeta elegida (handle del navegador)
  let carpetaEstado = 'nada';    // nada | lista | pedir | error
  let carpetaTexto = '';
  ...
  async function escribirEnCarpeta() {
    ...
    if (!(await permisoCarpeta(CARPETA, false))) { carpetaEstado = 'pedir'; return false; }
    const fh = await CARPETA.getFileHandle(ARCHIVO_RESPALDO, { create: true });
    const w = await fh.createWritable();
    await w.write(datosRespaldo());
    await w.close();
  ...
  function respaldarPronto() {
    ...
    tRespaldo = setTimeout(() => {
      escribirEnCarpeta().then((ok) => { if (ok) pintarCopia(); }).catch(() => { carpetaEstado = 'error'; });
    }, 4000);
  }
  ...
  async function importarDeCarpeta() {
    ...
    try {
      const fh = await CARPETA.getFileHandle(ARCHIVO_RESPALDO);
      const f = await fh.getFile();
      const r = importarMarcas(await f.text());
  ...
  async function conectarCarpeta(pedir) {
    try {
      ...
      if (!(await permisoCarpeta(CARPETA, !!pedir))) { carpetaEstado = 'pedir'; return false; }
      carpetaEstado = 'lista';
  ...
  function panelMarcasHTML() {
    ...
    return '<div class="sj-panel-in">' +
      ...
      '<p>' + (carpetaEstado === 'lista'
        ...
        : carpetaEstado === 'pedir'
          ? 'Hay una carpeta elegida (<b>' + esc(carpetaTexto) + '</b>), pero Chrome pide confirmar el permiso otra vez.'
          : carpetaEstado === 'error'
            ? 'Hubo un problema con la carpeta elegida. Probá elegirla de nuevo.'
  ...
  function alClic(e) {
    ...
    switch (a) {
      ...
      case 'conectarCarpeta':
        ...
        return;
      case 'guardarCarpeta':
        escribirEnCarpeta().then((ok) => { pintarCopia(); if (VISTA === 'marcas') irAVista('marcas'); avisar(ok ? 'Guardado en la carpeta.' : 'No se pudo guardar: revisá el permiso de la carpeta.', !ok); })
